          ...
          <div className="flex items-center gap-2">
              ...
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  className={`h-9 justify-between gap-2 min-w-[140px] ${
                    selectedRepo 
                      ? "hover:opacity-90 bg-foreground-color text-accent-color border-foreground-color" 
                      : "border-zinc-200 hover:bg-zinc-50 text-muted-text"
                  }`}
                  disabled={loading}
                >
                  {selectedRepoData && isLocalRepoPath ? (
                    <FolderOpen 
                      className={`h-3.5 w-3.5 flex-shrink-0 ${selectedRepo ? "text-accent-color" : "text-muted-text"}`}
                    />
                  ) : (
                    <Github 
                      className={`h-3.5 w-3.5 flex-shrink-0 ${selectedRepo ? "text-accent-color" : "text-muted-text"}`}
            ...
            <DropdownMenu open={branchDropdownOpen} onOpenChange={setBranchDropdownOpen}>
              <DropdownMenuTrigger asChild>
                ...
                >
                  <GitBranch 
                    className={`h-3.5 w-3.5 flex-shrink-0 ${selectedBranch ? "text-accent-color" : "text-muted-text"}`}
                  />
                  <span className="text-xs font-medium truncate max-w-[200px]">
              ...
              <Button
                ...
                className={`h-11 w-11 rounded-full shadow-sm transition-all duration-200 ${
                  loading || !input.trim() || !selectedRepo || !selectedBranch || !selectedAgent
                    ...
                    : "hover:scale-105 bg-foreground-color text-accent-color"
                }`}
                title={
                  !selectedRepo 
