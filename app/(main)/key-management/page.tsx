...
const KeyManagement = () => {
    ...
    const {
        ...
        queryFn: () =>
            axios
                .get<KeySecrets>(`${GlobalConfig.DBURL}/secrets/openai`)
                .then((res) => res.data)
    });
    ...
    const { mutate: deleteSecret, isPending: isDeleting } = useMutation({
        mutationFn: () => {
            return axios.delete(
                `${GlobalConfig.DBURL}/secrets/openai`
            );
        },
        onSuccess: () => {
    ...
    return (
        <div className="ml-8 mt-4 flex flex-col text-start h-full">
            <div className="flex flex-col pb-8">
                ...
                <h2 className="text-base text-start text-secondary pb-4">Select the key you want to use for generating tests</h2>
                <RadioGroup>
                    <div className="flex items-center">
                        <RadioGroupItem value="momentumKey" className="text-secondary" checked={keyType == "momentumKey"} onClick={() => setDeleteKeyDialogOpen(true)}></RadioGroupItem>
                        <Label className="pl-2">I want to use Momentums key</Label>
                    </div>
                    <div className="flex items-center">
                        <RadioGroupItem value="userKey" className="text-secondary" checked={keyType == "userKey"} onClick={() => setCreateNewKeyDialogOpen(true)}></RadioGroupItem>
                ...
                <Dialog open={deleteKeyDialogOpen} onOpenChange={setDeleteKeyDialogOpen}>
                    <DialogContent className="max-w-xl">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                <p className="text-base text-start text-secondary mt-2">
            ...
            <Dialog open={createNewKeyDialogOpen} onOpenChange={setCreateNewKeyDialogOpen}>
                <DialogContent className="max-w-xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            <p className="text-base text-start text-secondary mt-2">Select the LLM you want to use</p>
                            <p className="text-xs text-start text-secondary mt-1">Note: We only integrate with Open AI’s GPT-4o and GPT-3.5-Turbo. Support for integrating other LLMs will be available soon.</p>
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex flex-col">
                        ...
                        <div className="flex flex-col mt-2">
                            <h2 className="text-base text-start text-secondary">Enter your key</h2>
                            <Input type="password" 
                    ...
                    <Table className="">
                        <TableHeader>
                            <TableRow className="border-bottom border-border">
                                <TableHead className="w-[200px] text-secondary">Provider</TableHead>
                                <TableHead className="w-[200px] text-secondary text-right">Key Value</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
