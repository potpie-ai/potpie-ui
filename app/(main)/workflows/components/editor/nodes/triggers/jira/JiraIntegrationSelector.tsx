...
interface JiraIntegrationSelectorProps {
...
export const JiraIntegrationSelector = ({
    selectedIntegrationId,
    onIntegrationChange,
    readOnly = false,
}: JiraIntegrationSelectorProps) => {
    console.log(
        "JiraIntegrationSelector render with selectedIntegrationId:",
        selectedIntegrationId
    );
    ...
    const handleIntegrationChange = useCallback(
        (integrationId: string) => {
            if (readOnly) return;
            console.log(
                "JiraIntegrationSelector handleIntegrationChange called with:",
                integrationId
            );
            // Find the integration to get the unique identifier
            const integration = integrations.find((int) => int.id === integrationId);
            const uniqueIdentifier = integration?.uniqueIdentifier;
            console.log("JiraIntegrationSelector found integration:", integration);
            console.log(
                "JiraIntegrationSelector uniqueIdentifier:",
                uniqueIdentifier
            );
                                        ...
                                        ?.map((integration) => (
                                            ...
                                            >
                                                <div className="flex items-center gap-2 w-full">
                                                    ...
                                                    <div
                                                        className={`w-2 h-2 rounded-full ${integration.status === "active"
                                                                ? "bg-green-500"
                                                                : "bg-red-500"
                                                            }`}
                                                    />
